...
const LoginButton = () => {
  ...
  const [ displayName, setDisplayName ] = useState( t( "login" ) );
  ...
  const login = async ( email: string, password: string ) => {
    ...
    try{
      ...
      if( err instanceof Error )
      {
        ...
        console.error( err );
        setLoginErrorMessage( t( "unknownError" ) );
      }
    }
    finally{
      setLoading( false );
    ...
    <>
      {displayName === "admin" ?
        ...
        >
          ...
          <Menu.Dropdown>
            <Menu.Item onClick={ logout } icon={<IconLogout className="h-4 w-4" stroke={1.5} />}>{t( "logout" )}</Menu.Item>
          </Menu.Dropdown>
        </Menu>
        :
        <UnstyledButton
      ...
      <Modal opened={loginOpened} onClose={close} title={t( "loginTitle" )}>
