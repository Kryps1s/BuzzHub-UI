...
const JobForm = ( { trelloMembers, id } : JobFormProps ) : React.JSX.Element => {
  ...
  const form = useForm ( {
    ...
    validate: {
      participants: ( value ) => {
        if ( value.length === 0 ) {
          return t( "selectParticipants" );
        }
        if ( !leader ) {
          return t( "selectLeader" );
        }
        return null;
      }
    }
  ...
  const accordionItems = form.values.boxes.map( ( box, index ) => (
    <Accordion.Item value={box.box.toString()} key={box.box.toString() + index}>
      <Accordion.Control>{`${t( "box" )} #${box.box}`}</Accordion.Control>
      <Accordion.Panel>
        <BoxForm
          box={box}
          index={index}
  ...
  accordionItems.unshift(
    <Accordion.Item value="0" key="0">
      <Accordion.Control>{t( "general" )}</Accordion.Control>
      <Accordion.Panel>
        <Textarea
          className="px-6 py-2"
        ...
        <TextInput
          className="px-6 py-2"
          placeholder={t( "describeTemperment" )}
          label={t( "temperment" )}
          radius="sm"
          size="md"
        ...
        <DatesProvider settings={{ locale:getCookie( "NEXT_LOCALE" ) as string }}>
          <DateTimePicker
            className="px-6 py-2"
  ...
  return (
    <form>
      <Stepper id='stepper' className="px-4 max-w-4xl mx-auto h-4/5" active={active} onStepClick={goToStep}>
        ...
        <Stepper.Step label={t( "setup" )} description={t( "takeAttendance" )}>
          <div className="w-full max-w-xl mx-auto h-full px-4 ">
            <Title className="flex justify-center mb-4" order={2}>{t( "whoWasThere" )}</Title>
        ...
        <Stepper.Step label={t( "setup" )} description={t( "hivePrep" )}>
          <div className="flex flex-col align-middle h-full">
            <Title className="flex justify-center mb-4" order={2}>{t( "boxQty" )} </Title>
            <div className="flex flex-row justify-evenly">
              <div className="flex flex-col">
                <Title className="flex justify-center mb-4" order={2}>{t( "brood" )}</Title>
                <Quantity onChangeCallback={setBroodBoxes} min={1} max={3} value={getBoxTypeQuantity( form.values.boxes, "BROOD" as BoxType )} />
              </div>
              <div className="flex flex-col">
                <Title className="flex justify-center mb-4" order={2}>{t( "honey" )}</Title>
            ...
            {goal &&
              <div className="flex flex-col h-3/4">
                <Title className="flex justify-center mt-8" order={4}>{t( "lastInspection" )}:</Title>
        ...
        <Stepper.Step label={t( "inspection" )} id="notes" description={t( "Notes" )}>
          <div className="w-full max-w-xl mx-auto h-full px-4 ">
        ...
        <Stepper.Step label={t( "inspection" )} description={t( "nextSteps" )}>
          <div className="w-full flex flex-col max-w-xl mx-auto h-full px-4 ">
            <Title className="flex justify-center mb-4" order={2}>{t( "nextInspection" )}</Title>
            <DatePicker
        ...
        <Stepper.Completed>
          <div className="overflow-y-scroll w-full max-w-xl mx-auto h-full px-4 ">
            <Title order={2}> {t( "reportSubmitted" )}</Title>
          </div>
        </Stepper.Completed>
      </Stepper>
      ...
      <Group className="h-1/5 max-w-xl mx-auto my-auto" position="right" >
        ...
        {active ===4 && <Link href="/"><Button className="mr-4 mt-4 border bg-cyan-700 border-slate-200" >{t( "done" )}</Button></Link>}
        {active <3 && <Button id="nextStep" className="mr-4 mt-4 bg-cyan-700 border border-slate-200" onClick={() => goToStep( active+1 )}>{t( "nextStep" )}</Button>}
        {active === 3 && loading && <Loader/>}
        {active === 3 && <Button id="submitForm" className="mr-4 mt-4 bg-cyan-700 border border-slate-200" disabled={loading} onClick={submit}>{t( "submit" )}</Button>}
      </Group>
    </form>
  );
};
