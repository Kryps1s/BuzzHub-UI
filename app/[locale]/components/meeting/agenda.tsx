...
const MeetingAgenda = ( { agenda } : AgendaProps ) =>
{
  ...
  const accordionItems = ( Object.keys( agenda ) as Array<keyof typeof agenda> ).map( ( category ) => (
    <Accordion multiple defaultValue={[ t( "BEEKEEPING" ), t( "COLLECTIVE" ) ]} key={category}>
      <Accordion.Item id={`agenda-category-${category}`} value={`${category}`}>
        <Accordion.Control>{t( category )} <Badge>{
          ...
          <Accordion multiple>
            {Object.keys( agenda[category] as Category ).map( ( subCategory, subIndex: number ) => (
              <Accordion.Item
                ...
                key={`${subIndex}`}
              >
                <Accordion.Control>{t( subCategory )} <Badge>{agenda[category][subCategory].length}</Badge></Accordion.Control>
                <Accordion.Panel>
